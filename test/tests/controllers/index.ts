...
export const initControllerTests = (O:ControllerNetwork) => Jest.utils.desc("INIT",() => {
  it("Init DB & Controller Network",async () => {
    await Jest.utils.refreshDb();
    await O.init((await Jest.utils.initCore()).core);
  },1E9);
});
export const wrapupControllerTests = ({jsons}:ControllerNetwork) => Jest.utils.desc("WRAPUP",() => {
  it("Log Models",async () => {
